...
> 지저분한 CSV·엑셀을 올리면 **중복·포맷 오류·이상치·정합성 불일치**를 찾아내 정리하고 리포트해 주는 데이터 정제·검증 도구.

올린 파일은 서버로 보내지 않고 전부 브라우저(또는 Node) 안에서 처리합니다. AI를 쓰지 않는 규칙 기반이라, 같은 입력은 늘 같은 결과를 냅니다.

🔗 **[라이브 데모](https://duke-yoon.github.io/tidycsv/)** · **[GitHub](https://github.com/duke-yoon/tidycsv)**

> **상태:** ✅ v1 완성 — 코어 엔진(4종 검출) · 자동 정제·리포트 · 웹 UI · **GitHub Pages 라이브 배포**. [지금 바로 써보기 →](https://duke-yoon.github.io/tidycsv/)
...
회계·정산·마이그레이션처럼 **정합성이 중요한 작업**에서는 "합계가 안 맞는다 / 잔액이 안 맞는다 / 참조가 깨졌다 / 날짜 포맷이 섞였다" 같은 문제를 사람이 눈으로 잡기 어렵습니다. tidycsv는 이런 **정합성 검증을 코드로** 대신 해 주는 엔진입니다.

AI(LLM)를 쓰지 않아 **결과가 늘 일정**하고, 브라우저에서 돌아가 **민감한 데이터가 밖으로 나가지 않습니다.**
...
> 돈이 걸린 데이터를 임의로 "고치는" 건 위험하므로, 정합성 위반은 자동으로 바꾸지 않고 리포트의 "수동 검토" 항목으로 남깁니다. 월/일 순서를 단정할 수 없는 모호한 날짜(`05/06/2024` 등)도 건드리지 않습니다.
...
**테스트:** 220개 통과(코어 엔진·정제·엑셀 파싱·UI 스모크) · 함수 커버리지 100% · 의존성 취약점 0건.

---

## 알려진 한계

- 숫자 파싱: 영미식(`1,234.56`)과 유럽식(`1.234,56`)을 휴리스틱으로 자동 구분합니다. 콤마만 있는 `1,234`는 천단위로 간주하며, 모호한 경우의 판정 규칙은 `util/coerce.ts` 주석에 정리해 뒀습니다.
- 날짜 포맷: 연-후행 `12/05/2024` 형태는 월/일 순서를 단정할 수 없어 하나의 shape 로 묶고, 값으로 순서가 강제되는 경우(예: `25/03` ↔ `03/25`)만 충돌로 봅니다. 달력상 없는 날짜(`2024-02-30`, 윤년 아닌 `2023-02-29` 등)는 날짜로 보지 않습니다.
- 정제 시 공백 정규화는 셀 안의 줄바꿈·탭도 공백 한 칸으로 통일합니다(따옴표로 감싼 멀티라인 셀은 유의). 모든 변경은 before/after 로 리포트에 남아 확인할 수 있습니다.

---

## 완성된 기능 / 다음

- [x] 코어 정합성 엔진 — CSV·엑셀 파싱 + 4종 검출 + 단위 테스트
- [x] 자동 정제 + before/after 리포트
- [x] 웹 UI — 드래그앤드롭 업로드·결과 표시·CSV 다운로드 (브라우저 로컬 처리)
- [x] 한국어/영어 이중언어 + GitHub Pages 자동 배포
- [ ] (예정) 데모 GIF
